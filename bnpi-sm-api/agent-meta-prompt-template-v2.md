...
4. On meaningful features: `npm run feature:new -- <slug> owner:<your-agent> module` before large implementation.
...
This repository is the **Express modular API scaffold** (not the React SPA).
Companion frontend: `bnpi-sm-app` (separate repo/folder; clients use `VITE_API_BASE_URL`).

Architecture invariants for this project:
- Modules under `modules/<feature>/`; thin controllers; services own logic
- Zod schemas under `schema/`; env via `config/env.ts`
- `validateRequest` on mutating routes; document paths in `config/swagger.ts`
- Tests: Jest + Supertest under `tests/`
- Feature scaffold: `npm run feature:new -- <slug> title:Name owner:<agent> module`
...
* [PERFORMANCE / SECURITY / API CONTRACT CONSTRAINT]
...
* Code changes
* HTTP route / OpenAPI contract changes
...
9. Do not use passing unit tests as proof of a real production deploy, multi-region, or live dependency workflow.
...
* HTTP routes, validation schemas, and OpenAPI paths
