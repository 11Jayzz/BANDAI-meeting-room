#!/usr/bin/env node
/**
 * Open a delta checklist for updating an EXISTING feature.
 *
 * Creates ONLY workspace artifacts — never re-scaffolds module/app/swagger/tests.
 *
 * Always creates:
 *   .wwg/workspace/features/<slug>.update.md
 *   index row + current-task pointer
 *
 * Usage:
 *   npm run feature:update -- invoices
 *   npm run feature:update -- invoices summary:"Add GET list by status" owner:grok
 *   npm run feature:update -- invoices title:Invoices mode:bug-fix force
 *
 * Tokens:
 *   summary:… | --summary=
 *   title:…   | --title=
 *   owner:…   | --owner=
 *   mode:enhance|contract-change|bug-fix|schema-data|docs-only|mixed
 *   force
 *
 * Env:
 *   FEATURE_SCAFFOLD_ROOT  override project root (tests)
 *   FEATURE_TITLE, FEATURE_OWNER, FEATURE_SUMMARY, FEATURE_UPDATE_MODE, FEATURE_FORCE=1
 */
...
function parseArgs(argv) {
  ...
  if (compact.length) {
    ...
    // remaining free text as summary if summary not set
...
Does NOT scaffold or overwrite modules/app/swagger/tests.
Env: FEATURE_TITLE, FEATURE_OWNER, FEATURE_SUMMARY, FEATURE_UPDATE_MODE, FEATURE_FORCE=1, FEATURE_SCAFFOLD_ROOT`;
}
...
function fillUpdateChecklist({ template, slug, title, owner, summary, mode, id, moduleExists, parentExists }) {
  ...
  body = body.replace(
    ...
    `| Module path exists? | ${moduleExists ? "YES" : "NO"} — \`modules/${slug}/\` |`,
  );
  body = body.replace(
    "| Parent checklist | `.wwg/workspace/features/<slug>.md` (if any) |",
    parentExists
      ? `| Parent checklist | [\`${slug}.md\`](./${slug}.md) |`
      : `| Parent checklist | _(none)_ — optional: \`npm run feature:new -- ${slug}\` without module |`,
  );
...
function updateFeaturesIndex(slug, title, status = "UPDATE_IN_PROGRESS") {
  ...
  const row = `| ${slug} (update) | [\`${slug}.update.md\`](./${slug}.update.md) | ${status} |`;
...
function patchCurrentTask(slug, title, summary, updateFile) {
  ...
  const block = [
    ...
    `- Opened: ${today()}`,
    `- Rule: edit existing files only — **do not** \`feature:new … module force\``,
...
function detectModule(slug) {
  const moduleDir = path.join(root, "modules", slug);
  if (!fs.existsSync(moduleDir)) return { exists: false, files: [] };
  const files = fs.readdirSync(moduleDir).filter((f) => f.endsWith(".ts"));
  return { exists: true, files };
}

function printNextSteps(slug, title, flags, moduleInfo) {
  console.log(`
────────────────────────────────────────
Feature update ready: ${title} (${slug})
────────────────────────────────────────

Checklist: .wwg/workspace/features/${slug}.update.md
Mode:      ${flags.mode}
${flags.summary ? `Summary:   ${flags.summary}\n` : ""}Module:    ${
    moduleInfo.exists
      ? `found modules/${slug}/ (${moduleInfo.files.join(", ") || "…" })`
      : `NOT FOUND — if this is a brand-new feature use:\n             npm run feature:new -- ${slug} title:${title} owner:${flags.owner} module`
  }

Next (docs/FEATURE_WORKFLOW.md → Feature update):
  1. Fill delta intent + acceptance in the update checklist
  2. Edit existing files only (module/schema/swagger/API.md/tests/db)
  3. Tests: enhance → happy+400; bug-fix → regression
  4. npm run test && npm run check
  5. npm run wwg:validate && npm run wwg:brief

Never re-scaffold with: feature:new … module force
`);
}
...
function main() {
  ...
  const moduleInfo = detectModule(slug);
  const parentChecklist = path.join(
    root,
    ".wwg",
    "workspace",
    "features",
  ...
  const parentExists = fs.existsSync(parentChecklist);

  console.log(`\nOpening feature update "${title}" (${slug})…\n`);

  if (!moduleInfo.exists) {
    console.log(
      `  ⚠ modules/${slug}/ not found.\n` +
        `    If this is NEW work, prefer: npm run feature:new -- ${slug} title:${title} owner:${owner} module\n` +
  ...
  const checklist = fillUpdateChecklist({
    ...
    mode,
    id,
    moduleExists: moduleInfo.exists,
    parentExists,
  });
  ...
  updateFeaturesIndex(slug, title, "UPDATE_IN_PROGRESS");
  patchCurrentTask(slug, title, summary, checklistRel);

  printNextSteps(slug, title, { ...flags, mode }, moduleInfo);
}
